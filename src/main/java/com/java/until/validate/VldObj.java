...
public class VldObj {
	...
	private void vldField(Object vldObj, VldField vldField, Class<?> clazz) {
		if (null != vldField && null != vldField.getValidate()) {
			...
			// 是否必填
			String vldStr = StringUtils.toString(vldObj);
			if (StringUtils.isNull(vldStr) && vld.required()) {
				vldMsg.append(String.format(VldProperty.get("REQUIRED_MSG"), vldField.getFieldName()));
			}
	...
	private void vldFieldType(String vldStr, String fieldMsg, ValidateType validateType) {
		...
		if (null != valMethod) {
			try {
				boolean bl = StringUtils.toBoolean(valMethod.getMethod().invoke(null, vldStr));
				// 校验失败
				if (!bl) {
					// 获取配置文件内容
					String msgProperty = valMethod.getVldMethod().msgProperty();
					if (!StringUtils.isNull(msgProperty)) {
			...
			for (Field field : fields) {
							...
							for (String vldName : vldNames) {
								if (StringUtils.isNull(vldName)) {
									...
									for (String pattern : patternList) {
										if (StringUtils.wildMatch(pattern, vldName)) {
											isVld = true;
											break;
										}
									}
				...
				if (StringUtils.isNull(fieldName)) {
					...
					if (FIELD_TYPE_CASE == 2) {
						fieldName = StringUtils.toCapitalizeCamelCase(fieldName);
				...
				if (!StringUtils.isNull(parentName)) {
